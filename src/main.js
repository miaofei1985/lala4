...
const STORAGE_KEY = "ad-affiliate-transparency-board:v2";
...
function isSafeUrl(v) { return /^https?:\/\//i.test(String(v || "")); }
...
function advertiserCell(r) { const name = escapeHtml(r.advertiser); const url = isSafeUrl(r.url) ? escapeHtml(r.url) : ""; const link = url ? `<a class="advertiser-link" href="${url}" target="_blank" rel="noopener noreferrer">${name}<span>打开网站 ↗</span></a>` : `<strong>${name}</strong>`; return `${link}<small>${escapeHtml(r.source)}</small>`; }
function row(r) { return `<tr><td>${advertiserCell(r)}</td><td>${escapeHtml(r.project)}<small>${escapeHtml(r.requirements)}</small></td><td><span class="pill">${escapeHtml(r.category)}</span></td><td>${escapeHtml(r.model)}</td><td class="price">${escapeHtml(r.price)}<small>最低提现：${escapeHtml(r.minPayout)}｜归因：${escapeHtml(r.cookie)}</small></td><td>${escapeHtml(r.geo)}</td><td>${badge(r.transparency)}</td><td>${escapeHtml(r.updatedAt)}</td></tr>`; }
...
function render() {
  ...
  app.innerHTML = `<header class="hero"><nav class="nav"><div class="brand">广告联盟透明报价台</div><a class="github-link" href="https://github.com/miaofei1985/lala4" target="_blank" rel="noreferrer">GitHub 项目</a></nav><section class="hero-grid"><div><p class="eyebrow">Affiliate Price Intelligence</p><h1>把广告主项目、计费模式和价格状态放到一个透明页面。</h1><p class="subtitle">适合广告联盟、导购站、流量主、运营团队维护广告主报价库。所有价格都带透明度、来源、官网链接和更新时间。</p><div class="actions"><button id="exportCsv">导出 CSV</button><label class="upload-button">导入 CSV<input id="importCsv" type="file" accept=".csv,text/csv" hidden /></label><button class="secondary" id="resetData">恢复样例数据</button></div></div><div class="notice-card"><strong>数据原则</strong><p>广告价格会随国家、流量质量、类目和合同变化。页面会明确标注“公开、需申请、联系商务、待核实”。点击广告主名称可打开对应官网或联盟入口。</p></div></section></header><main class="container"><section class="stats-grid">${stat("当前结果", rows.length, "条广告项目")}${stat("公开报价", stats.pub, "官网或后台可核实")}${stat("需申请", stats.apply, "后台批准后可见")}${stat("联系商务", stats.biz, "合同或商务报价")}</section><section class="filters"><input id="search" type="search" placeholder="搜索广告主、联盟、项目、国家、网址、要求..." value="${escapeHtml(state.search)}" />${select("category", uniqueOptions("category", "全部行业"), state.category)}${select("model", ["全部模式", "CPC", "CPM", "CPA", "CPS", "CPL", "RevShare", "收入分成", "佣金分成"], state.model)}${select("transparency", uniqueOptions("transparency", "全部透明度"), state.transparency)}</section><section class="table-card"><div class="table-header"><div><h2>广告主项目与价格表</h2><p>点击广告主名称可访问官网/联盟入口；点击表头可排序。</p></div><span>${stats.cats} 个行业分类</span></div><div class="table-wrap"><table><thead><tr>${th("advertiser", "广告主 / 官网")}${th("project", "项目")}${th("category", "行业")}${th("model", "计费")}${th("price", "价格 / 佣金")}${th("geo", "地区")}${th("transparency", "透明度")}${th("updatedAt", "更新")}</tr></thead><tbody>${rows.map(row).join("") || `<tr><td colspan="8" class="empty">没有匹配结果，请调整筛选条件。</td></tr>`}</tbody></table></div></section><section class="guide-grid"><article><h3>怎样做到明明白白</h3><p>每条数据都必须有价格说明、来源、更新时间、官网链接和透明度。没有官方价格的广告主不要瞎填。</p></article><article><h3>后续可接入</h3><p>Supabase / PostgreSQL 管理后台、定时价格更新、报价截图存档、广告主审核状态。</p></article><article><h3>CSV 字段</h3><p>advertiser, url, project, category, geo, model, price, minPayout, cookie, requirements, transparency, source, updatedAt</p></article></section></main>`;
  bindEvents();
}
...
function exportCsv() { const headers = ["advertiser", "url", "project", "category", "geo", "model", "price", "minPayout", "cookie", "requirements", "transparency", "source", "updatedAt"]; const lines = [headers.join(","), ...state.rows.map((r) => headers.map((h) => csvEscape(r[h])).join(","))]; const url = URL.createObjectURL(new Blob([lines.join("\n")], { type: "text/csv;charset=utf-8" })); const a = document.createElement("a"); a.href = url; a.download = "advertisers.csv"; a.click(); URL.revokeObjectURL(url); }
